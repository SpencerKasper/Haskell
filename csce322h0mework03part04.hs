...
main = do
     ...
     print ("Result")
     printGame (manyPlayersManyMoves game moves (selectPlayers game (generatePositions game) 2))


selectPlayers :: [[Char]] -> [(Int, Int)] -> Int -> [Char]
-- selectPlayers game positionsToCheck max
selectPlayers game [] 4 = "1234"
selectPlayers game [] 3 = "123"
selectPlayers game [] 2 = "12"
selectPlayers game (h:t) max
		| ((charToInt (get game h)) == 4)               = "1234"
		| ((charToInt (get game h)) == 3) && (max <= 3) = selectPlayers game t 3
		| otherwise                                     = selectPlayers game t max
...
manyPlayersManyMoves :: [[Char]] -> [Int] -> [Char] -> [[Char]]
manyPlayersManyMoves game [] _                = game
manyPlayersManyMoves game (move:moves) player
		| ((checkWin game positionsToCheck (0,0)) == true) = game
		| otherwise = manyPlayersManyMoves (changeCluster game (findCluster game [] [(0,0)] move 1) (head (player))) moves (incrementPlayer player)
		where positionsToCheck = [n|n<-(generatePositions game), (positionsToCheck != 0)]

incrementPlayer :: [Char] -> [Char]
incrementPlayer (h:t) = (t ++ [h])


checkWin :: [[Char]] -> [(Int, Int)] -> (Int, Int) -> Bool
checkWin game [] _ = true
checkWin game (h:t) previousTuple
		| ((get game h) == ((get game previousTuple)) = checkWin game t head
		| otherwise = false
